...
void setup() 
{
    ...
    Serial.begin(250000);                                           // INICIO DE CONFIGURACION SERIAL
      
    keyboard.begin(DataPin, IRQpin);                                // INICIALIZACION DE TECLADO DE PC   
    ...
    while (!keyboard.available()) {
      reiniciar_teclado_lcd();
      delay(250); // Espera 100 milisegundos antes de volver a verificar
    }
...
#ifdef DEBUG 
    Serial.println("************* Validacion de clave ***************");   
    Serial.print("Clave Ingresada = ");   
    Serial.println(Datos_clave);
